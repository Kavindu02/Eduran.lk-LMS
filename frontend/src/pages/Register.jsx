...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#06120c] flex items-center justify-center py-8 sm:py-12 md:py-20 px-4 sm:px-6 relative overflow-hidden font-sans">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        {/* Decorative Blurs */}
        <div className="absolute top-[-10%] right-[-10%] w-64 sm:w-96 h-64 sm:h-96 bg-emerald-500/20 rounded-full blur-[80px] sm:blur-[100px] animate-pulse" />
        <div className="absolute bottom-[-10%] left-[-10%] w-64 sm:w-96 h-64 sm:h-96 bg-emerald-400/10 rounded-full blur-[80px] sm:blur-[100px]" />
      </div>
      ...
      <HydrationWrapper className="w-full max-w-5xl relative z-10">
        ...
        <div className="flex flex-col items-center mb-8 sm:mb-10 animate-fadeIn text-center">
          ...
          <p className="text-white/60 font-serif italic text-base sm:text-lg mt-2 underline decoration-emerald-500/30 underline-offset-4">
            ...
            <form onSubmit={handleRegister} className="space-y-8 sm:space-y-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 sm:gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center bg-white/5 border border-white/10 rounded-lg overflow-hidden transition-all h-9 w-full sm:w-32 lg:w-full">
                      <button 
                        type="button" 
                        className="w-8 h-full flex items-center justify-center text-emerald-500 hover:bg-emerald-500 hover:text-black transition-all border-r border-white/10"
                        onClick={() => {
                      ...
                      <button 
                        type="button" 
                        className="w-8 h-full flex items-center justify-center text-emerald-500 hover:bg-emerald-500 hover:text-black transition-all border-l border-white/10"
                        onClick={() => {
                            ...
                            return (
                                ...
                                >
                                  <div className="flex items-center gap-4 flex-1">
                                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center transition-all duration-500 ${isSelected ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20 rotate-[360deg]' : 'bg-white/5 text-white/40'}`}>
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[9px] sm:text-[10px] text-emerald-500/60 font-black tracking-widest uppercase mt-1 leading-none">{sub.code || 'CODE ---'}</p>
                                    </div>
                                  </div>
                                  <div className="ml-3 sm:ml-4 flex-shrink-0 pointer-events-none">
                                    <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-lg border-2 transition-all duration-300 flex items-center justify-center ${isSelected ? 'bg-emerald-500 border-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-transparent border-white/20'}`}>
                              ...
                              return (
                                <div key={`t-sel-${sid}`} className="bg-black/40 border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-5 space-y-4 animate-fadeIn">
                                  <div className="flex items-center justify-between pb-3 border-b border-white/5">
                                    <h4 className="text-emerald-500 font-black uppercase tracking-widest text-[9px] sm:text-[10px]">{sub.name || 'Subject'} Teachers</h4>
                                    <span className="bg-emerald-500/10 text-emerald-500 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter">Choose One or More</span>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
                                    {subTeachers.map(t => {
                                      ...
                                      return (
                                        <div 
                                          key={`t-opt-${sid}-${t.id}`} 
                                          className={`group flex items-center justify-between p-3.5 rounded-xl sm:rounded-2xl border-2 transition-all duration-300 cursor-pointer select-none ${isChosen ? 'bg-emerald-500/20 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'bg-white/5 border-transparent hover:bg-white/10 hover:border-white/10'}`}
                                          onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        >
                                          <div className="flex items-center gap-3.5 flex-1 overflow-hidden">
                                            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${isChosen ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20' : 'bg-white/5 text-white/40'}`}>
                                              <User className="w-4.5 h-4.5 sm:w-5 sm:h-5 flex-shrink-0" />
                                            </div>
                                            <div className="overflow-hidden">
                                              <p className="text-[11px] sm:text-[12px] font-black text-white hover:text-emerald-500 transition-colors uppercase truncate tracking-tight mb-0.5">{t.name}</p>
                                              <p className="text-[8px] sm:text-[9px] text-emerald-500/40 font-black tracking-[0.2em] uppercase truncate">{t.qualification || 'LEVEL EXPERT'}</p>
                                            </div>
                                          </div>
                                          <div className="ml-3 flex-shrink-0 pointer-events-none">
                                            <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-lg border-2 transition-all duration-300 flex items-center justify-center ${isChosen ? 'bg-emerald-500 border-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-transparent border-white/20'}`}>
              ...
              <div className="pt-8 w-full">
                <Button 
                  type="submit" 
                  disabled={loading} 
                  className="w-full h-14 sm:h-16 bg-white hover:bg-emerald-500 text-black hover:text-white font-black uppercase tracking-[0.25em] sm:tracking-[0.3em] text-xs sm:text-sm transition-all duration-500 rounded-2xl group relative overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.3)] shadow-emerald-500/0 hover:shadow-emerald-500/20 active:scale-[0.98]"
                >
  ...
  return (
    <div className={`space-y-2 group ${isFullWidth ? 'sm:col-span-2' : ''}`}>
      ...
      <div className="relative">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30 group-focus-within:text-emerald-500 transition-colors pointer-events-none">
          {icon}
        </div>
        <Input 
          id={id} type={type} placeholder={placeholder} value={value} onChange={onChange} required
          className="h-12 sm:h-13 bg-white/5 border-white/10 pl-12 text-white placeholder:text-white/10 focus:ring-emerald-500/20 focus:border-emerald-500/50 transition-all rounded-xl text-xs sm:text-sm"
        />
      </div>
    </div>
  );
