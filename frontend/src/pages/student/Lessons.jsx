            ...
            try {
                ...
                if (res.ok) {
                    ...
                    (data.selectedSubjects || []).forEach(item => {
                        ...
                        // Treat NULL teacherId as a special case or skip if it's strictly teacher-based
                        if (item.teacherId) {
                            ...
                            // If no teacher is assigned, we record the payment status for the subject itself
    ...
    return (
        <ProtectedLayout requiredRole="student" title="My Lessons">
            <div className="space-y-6 md:space-y-10 animate-fadeIn">
                ...
                {/* Modern Hero Section */}
                <div className="relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-white border border-slate-200 p-6 md:p-12 shadow-2xl">
                    <div className="absolute top-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-emerald-500/5 rounded-full blur-[60px] md:blur-[80px] -mr-24 -mt-24 md:-mr-32 -mt-32" />
                    <div className="absolute bottom-0 left-0 w-48 h-48 md:w-64 md:h-64 bg-emerald-400/5 rounded-full blur-[60px] md:blur-[80px] -ml-24 -mb-24 md:-ml-32 -mb-32" />
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
                        <div className="space-y-2">
                            <div className="flex items-center gap-3 mb-1 md:mb-2">
                                ...
                                <span className="text-[8px] md:text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em]">Course Materials</span>
                            </div>
                            <h1 className="text-2xl md:text-5xl font-black text-slate-800 tracking-tighter uppercase leading-none">
                                Your <span className="text-emerald-500">Learning</span> Library
                            </h1>
                            <p className="text-slate-400 font-medium uppercase tracking-[0.2em] text-[8px] md:text-[10px]">
                                Pick a subject and continue where you left off
                            </p>
                        </div>
                        
                        <div className="flex md:hidden lg:flex items-center gap-4 md:gap-6">
                            <div className="text-left md:text-right">
                                <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Total Subjects</p>
                                <p className="text-xl md:text-2xl font-black text-slate-800">{userSubjects.length}</p>
                            </div>
                            <div className="w-px h-8 md:h-10 bg-slate-100" />
                            <div className="text-left md:text-right">
                                <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Available Content</p>
                                <p className="text-xl md:text-2xl font-black text-emerald-500">{videos.length} Videos</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
                    ...
                    <div className="lg:col-span-8 space-y-4">
                        <div className="flex items-center justify-between px-2">
                            <label className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600">Select Subject</label>
                            <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{userSubjects.length} Registers</span>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                            {userSubjects.map(sub => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`group relative p-4 md:p-5 rounded-[1.5rem] md:rounded-3xl border-2 transition-all duration-300 text-left overflow-hidden ${
                                            isSelected 
                            ...
                            {userSubjects.length === 0 && (
                                <p className="col-span-full text-slate-400 text-[10px] md:text-[11px] font-bold uppercase tracking-widest p-10 md:p-12 bg-white border-2 border-dashed border-slate-100 rounded-[2rem] text-center">
                ...
                <div className="space-y-6 md:space-y-8 pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-3 md:gap-4">
                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-slate-900 flex items-center justify-center shadow-lg">
                                <VideoIcon className="w-4 h-4 md:w-5 md:h-5 text-emerald-500" />
                            </div>
                            <div>
                                <h2 className="text-lg md:text-2xl font-black text-slate-800 uppercase tracking-tighter leading-none">
                                    Available Lessons
                                </h2>
                                <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 md:mt-1">High Quality Video Content</p>
                            </div>
                        </div>
                        {videos.length > 0 && (
                            <div className="px-3 md:px-4 py-1 md:py-1.5 bg-emerald-500 text-white rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 whitespace-nowrap">
